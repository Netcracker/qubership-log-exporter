...
func (h *CustomHistogram) Observe(sum float64, cnt uint64, buckets map[float64]uint64, labels map[string]string, labelKeys []string, timestamp *time.Time) {
    ...
    defer h.Unlock()
    if cnt < 0 {
        log.WithField(ec.FIELD, ec.LME_1003).Errorf("Value for cnt is less than 0 : %v ; Skipping histogram update", cnt)
        return
    }
    ...
    for key,value := range buckets {
        if value < 0 {
            log.WithField(ec.FIELD, ec.LME_1003).Errorf("Value for bucket %v is less than 0 : %v ; Skipping histogram %v update", key, value, histKey)
            return
        }
