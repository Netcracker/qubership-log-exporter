...
func ValidateConfig(config *Config) error {
    ...
    if len(config.Metrics) == 0 {
        startupBlockingErrors = append(startupBlockingErrors, fmt.Sprintf("Section metrics : No metrics are specified"))
    }

    if len(config.Queries) == 0 {
        startupBlockingErrors = append(startupBlockingErrors, fmt.Sprintf("Section queries : No queries are specified"))
    }
    ...
    for metricName, metricConfig := range config.Metrics {
        ...
        for metricName := range config.Metrics {
            if queryMetrics[metricName] == 0 && childMetrics[metricName] == 0 {
                ...
                log.Warnf("Section metrics : Metric %v is evaluated by %v query and at the same time is a child of %v metric, it may cause undefined behaviour", metricName, queryMetrics[metricName], childMetrics[metricName])
            } else if queryMetrics[metricName] > 1 {
                log.Warnf("Section metrics : Metric %v is evaluated by %v queries at the same time, it may cause undefined behaviour", metricName, queryMetrics[metricName])
            } else if childMetrics[metricName] > 1 {
                log.Warnf("Section metrics : Metric %v is a child of %v metrics at the same time, it may cause undefined behaviour", metricName, childMetrics[metricName])
            }
        }
...
func performQueriesNonBlockingChecks(config *Config) {
    isNewRelic := false
    if config.Datasources[config.DsName].Type == "newrelic" {
        isNewRelic = true
    }
