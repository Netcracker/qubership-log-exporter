...
func enrichFromEnvironmentVariables(config *Config) {
    ...
    for _, exportConfig := range config.Exports {
        ...
        if exportConfig.Consumer == "victoria-vmagent" || exportConfig.Consumer == "" {
...
func processHiddenFields(config *Config) {
    ...
    for queryName, queryConfig := range config.Queries {
        ...
        if err != nil {
            log.WithField(ec.FIELD, ec.LME_8102).Errorf("Error parsing timelag duration %v for query %v : %+v", queryConfig.QueryLag, queryName, err)
            queryLag = -1
        }
    ...
    for metricName, metric := range config.Metrics {
        for _, label := range metric.LabelsInitial {
