...
func TestEvaluateMetric(t *testing.T) {
    ...
    for _, csv := range testCfg.GraylogEmulator.Data {
        emuData, err, _ := httpservice.ProcessCsv(csv, "test_query")
        if err != nil {
    ...
    //printMetric("graylog_messages_gauge_total_by_host_by_container#2", testCfg, gds)
...
func printMetric(testName string, testCfg *config.Config, gds []*queues.GraylogData) {
    currTime := time.Now()
    metricName, gdIndex, err := getMetricAndIndex(testName)
    if err != nil {
        fmt.Print(err.Error())
    ...
    e := CreateEvaluator(testCfg)
    metricConfig := testCfg.Metrics[metricName]
    mer := e.EvaluateMetric(gds[gdIndex].Data, metricName, metricConfig, "any", &currTime)

    fmt.Printf("    %v :\n", testName)
    for _, ms := range mer.Series {
        fmt.Printf("%v_%v_%v_%+v\n", ms.Average, ms.Count, ms.Sum, ms.Labels)
    }
}
