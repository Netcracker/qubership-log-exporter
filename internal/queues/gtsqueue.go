...
type GTSQueue struct { // Graylog timeranges start queue
...
func (gtsq *GTSQueue) generateHistoryTimestamps() {
    ...
    for queryName, queryConfig := range gtsq.appConfig.Queries {
        queryName := queryName; queryConfig := queryConfig
        ...
        go func() {
            ...
            for i := 0; i < retryCount ; i++ {
                unixTime, retryPossible, err, errc = gtsq.lastTimestampService.GetLastTimestampUnixTime(queryName, queryConfig)
                if err == nil {
                    log.Infof("NewGTSQueue : For query %v attempt %v of %v to extract last timestamp succeded", queryName, i + 1, retryCount)
                    break
...
func (gtsq *GTSQueue) scheduleTimestampGenerationForQuery(queryName string) {
    ...
    if err != nil {
        ...
        log.WithField(ec.FIELD, ec.LME_1608).Errorf("During registering query %v in croniter following errror occurred : %+v", queryName, err)
    } else {
